...
class Product extends JsonResource
{
    ...
    public function toArray($request)
    {
        try {
            ...
            $cat = DB::table('product_categories')->where('product_id', $product->id)->first();

            if ($cat) {
                $category = DB::table('category_translations')->find($cat->category_id);
            } else {
                $category = [];
            }
            ...
            $wishlist = false;
            if ($customer) {
                $is_wishlist_list = DB::table('wishlist')
            ...
            return [
                ...
                'category' => $category,
                'price' => (float)$productTypeInstance->getMinimalPrice(),
    ...
    private function specialPriceInfo()
    {
        ...
        return [
            'special_price'          =>  $this->when(
                ...
                (float) $productTypeInstance->getSpecialPrice()
            ),
            ...
            'regular_price'          => $this->when(
                $productTypeInstance->haveSpecialPrice(),
                (float) data_get($productTypeInstance->getProductPrices(), 'regular_price.price')
            ),
